...
static int slm_lwm2m_read_uicc(void)
{
...
static int do_lwm2m_op(enum at_cmd_type cmd_type, const struct at_param_list *param_list,
		       uint32_t param_count)
{
	...
	case AT_CMD_TYPE_SET_COMMAND:
		...
		err = at_params_int_get(param_list, 1, &op);
		if (err) {
			return err;
		}
		...
		case 10: /* Read UICC LwM2M Bootstrap data */
			err = slm_lwm2m_read_uicc();
			break;
...
static int handle_at_lwm2m_data(enum at_cmd_type cmd_type, const struct at_param_list *param_list,
				uint32_t param_count)
{
	...
	case AT_CMD_TYPE_SET_COMMAND:
		if (param_count > 1) {
			err = at_params_unsigned_short_get(param_list, 1, &obj_inst_id);
			if (err) {
				return err;
			}
		}
		...
		if (param_count > 2) {
			err = at_params_unsigned_short_get(param_list, 2, &res_inst_id);
			if (err) {
				return err;
			}
		}
		...
		if (param_count < 4) {
			...
			err = at_params_string_ptr_get(param_list, 3, &hex_data, &hex_data_len);
			if (err) {
				return err;
			}
