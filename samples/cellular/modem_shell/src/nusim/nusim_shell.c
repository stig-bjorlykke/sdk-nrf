...
{
	uint8_t c_apdu[COS_MAX_MSG_SIZE];
	uint8_t r_apdu[COS_MAX_MSG_SIZE];
	...
	res = rt_cos_api_send_recv_msg(IO_CMD_TYPE_DATA, c_apdu, c_apdu_len, r_apdu, &r_apdu_len);
	if (res != COS_SUCCESS) {
...
static int cmd_nusim_factory_reset(const struct shell *shell, size_t argc, char **argv)
{
	mosh_print("nuSIM factory reset");
	rt_cos_api_factory_reset();
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_nusim_reset,
	...
	SHELL_CMD(factory, NULL, "Factory reset", cmd_nusim_factory_reset),
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_nusim,
	...
	SHELL_CMD(reset, &sub_nusim_reset, "nuSIM reset", NULL),
